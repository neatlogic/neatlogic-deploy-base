...
public class DeployJobCannotExecuteException extends ApiRuntimeException {
    ...
    public DeployJobCannotExecuteException(DeployJobVo deployJobVo) {
        super("无法执行，当前用户没有当前应用系统/环境/场景 '"+ deployJobVo.getAppSystemName()+"("+deployJobVo.getAppSystemId()+")/"+deployJobVo.getEnvName()+"("+deployJobVo.getEnvId()+")/"+deployJobVo.getScenarioName()+"("+deployJobVo.getScenarioId()+")' 权限" );
    }
}
